...
Next, we need to attribute each read to a restriction fragment or bin. The first step is to compute the restriction map of the genome.
...
cooler digest chrom.sizes genome.fa DpnII > fragments.bed
```
...
```bash
cooler cload pairs -c1 2 -p1 3 -c2 4 -p2 5 fragments.bed valid_pcrfree.pairs demo.cool
```
...
```bash
bwa mem -5SP genome.fasta for.fq rev.fq | samtools view -S -h -b -F 2316 > contacts.bam
pairtools parse contacts.bam > contacts.pairs
```
