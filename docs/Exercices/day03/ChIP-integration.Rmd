...
require(rtracklayer)
list_TF_peaks <- parallel::mclapply(mc.cores = 1, TFs, function(TF) {
...
all_peaks <- GenomicRanges::reduce(do.call(c, lapply(
    list.files(pattern = '.*narrowPeak$', path = 'results/bwa/mergedLibrary/macs/narrowPeak', full.names = TRUE), rtracklayer::import
)))
```
</p></details><br>
...
> Calculate the odds ratio of (1) binding of Abf1 and (2) binding of Reb1 over all the bound peaks  

<details><summary style="color: #ff7f00; font-weight: bold">Show code</summary><p>
```{r}
TF1 <-'Abf1'
TF2 <-'Reb1'
peaks_with_both <- sum(df[, 'Abf1'] & df[, 'Reb1'])
peaks_with_1 <- sum(df[, 'Abf1'] & !df[, 'Reb1'])
peaks_with_2 <- sum(!df[, 'Abf1'] & df[, 'Reb1'])
peaks_with_none <- sum(!df[, 'Abf1'] & !df[, 'Reb1'])
